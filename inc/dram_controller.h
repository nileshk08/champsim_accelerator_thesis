...
#define DRAM_WQ_SIZE 512 
#define DRAM_RQ_SIZE 512
...
class MEMORY_CONTROLLER : public MEMORY {
  public:
    ...
    uint64_t bank_cycle_available[DRAM_CHANNELS][DRAM_RANKS][DRAM_BANKS], no_of_PTW;
    ...
    // queues
    PACKET_QUEUE WQ[DRAM_CHANNELS], RQ[DRAM_CHANNELS],PROCESSED[NUM_CPUS];

    // constructor
    MEMORY_CONTROLLER(string v1) : NAME (v1) {
	    for(int i=0;i<(int)NUM_CPUS;i++){
		    cout << "NUM of cpus in dram " << NUM_CPUS << endl;

		    PROCESSED[i].NAME = "DRAM_PROCESSED_" + to_string(i);
		    PROCESSED[i].SIZE = ROB_SIZE;
		    PROCESSED[i].entry = new PACKET [ROB_SIZE];
	    }
