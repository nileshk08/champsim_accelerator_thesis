...
if [ "$#" -lt 12 ] || [ "$#" -gt 12 ]; then
    ...
    echo "Usage: ./run_4core.sh [BINARY] [N_WARM] [N_SIM] [N_MIX] [TRACE0] [ASID0] [TRACE1] [ASID1] [TRACE2] [ASID2] [TRACE3] [ASID3]"
    exit 1
fi
...
if ! [[ $ASID0 =~ $re ]] || ! [[ $ASID1 =~ $re ]] || ! [[ $ASID2 =~ $re ]] || ! [[ $ASID3 =~ $re ]]  ; then
...
(./bin/${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 ${OPTION} -traces ${TRACE_DIR}/${TRACE0} ${ASID0} ${TRACE_DIR}/${TRACE1} ${ASID1} ${TRACE_DIR}/${TRACE2} ${ASID2} ${TRACE_DIR}/${TRACE3} ${ASID3} ) &> results_4core_${N_SIM}M/mix${N_MIX}-${BINARY}.txt
