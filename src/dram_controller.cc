...
/*	if(current_core_cycle[0] > 19942687){
		for(int i=0;i<DRAM_CHANNELS;i++){
			cout << "write mode " << (int) write_mode[i] << "RQ next shceuule index " << RQ[0].next_schedule_index <<"   channel i " << i << " current cycle " << current_core_cycle[0]  << "RQ occupancy " << RQ[i].occupancy<< endl;
			RQ[i].queuePrint();
			cout << "WQ occupancy " << WQ[i].occupancy << endl;
			WQ[i].queuePrint();
		}
	}
  */    //         cout << "occupancy RQ[i] " << RQ[i].occupancy << " i " << i  << endl;
         //       RQ[i].queuePrint();
        // handle write
        // schedule new entry
    ...
    if (bank_request[op_channel][op_rank][op_bank].cycle_available <= current_core_cycle[op_cpu]) {
		...
		if(op_cpu >= ACCELERATOR_START){
			...
			if(queue->entry[request_index].type == TRANSLATION || queue->entry[request_index].type == PREFETCH){
				...
				upper_level_icache[queue->entry[request_index].cpu]->return_data(&queue->entry[request_index]);
			}
		/*	if(queue->entry[request_index].type == TRANSLATION)
				extra_interface -> return_data(&queue -> entry[request_index]);
			else
				PROCESSED[op_cpu].add_queue(&queue->entry[request_index]);*/
                }
		else{
...
int MEMORY_CONTROLLER::add_rq(PACKET *packet)
{
    ...
    if (all_warmup_complete < NUM_CPUS) {
	    if(packet->cpu >= ACCELERATOR_START){
			...
			SCRATCHPADPRINT(cout << "add from dram 2  trans " << (int) packet->type  << " cpu " << packet->cpu << " instr id " << packet->instr_id << endl;)
   ...
   /* if(packet->cpu >= ACCELERATOR_START && packet -> type != TRANSLATION){ // for accelerator, if request for data comes just return it by adding constant time
    	packet-> event_cycle = current_core_cycle[packet->cpu] + 80;
	SCRATCHPADPRINT(cout << "add from dram 5  trans " << (int) packet->type  << " cpu " << packet->cpu << " instr id " << packet->instr_id << endl;)
	upper_level_icache[packet->cpu]->return_data(packet);
	return -1;
    }*/
    ...
    if(packet->type != TRANSLATION);
	 //   index = check_dram_queue(&RQ[channel], packet);
    if (index != -1){

            if (packet->instruction) {
                    uint32_t rob_index = packet->rob_index;
                    RQ[channel].entry[index].rob_index_depend_on_me.insert (rob_index);
                    RQ[channel].entry[index].instr_merged = 1;
            ...
            else
            {
                    ...
                    if (packet->type == RFO) {
                            ...
                            RQ[channel].entry[index].sq_index_depend_on_me.insert (sq_index);
                            RQ[channel].entry[index].store_merged = 1;
                    }
                    else {
                            uint32_t lq_index = packet->lq_index;
                            RQ[channel].entry[index].lq_index_depend_on_me.insert (lq_index);
                            RQ[channel].entry[index].load_merged = 1;
                        }
            }
        RQ[channel].MERGED++;
        RQ[channel].ACCESS++;
...
//	   cout << "inside add wq for dram packet instr id " << packet->instr_id << " addr " << packet->address<< " channel "<< channel  <<endl;
//	   WQ[channel].queuePrint();
...
int MEMORY_CONTROLLER::check_dram_queue(PACKET_QUEUE *queue, PACKET *packet)
{
    ...
    for (uint32_t index=0; index<queue->SIZE; index++) {
        if ((queue->entry[index].address == packet->address) && (queue->entry[index].cpu == packet->cpu) ) {
            
            DP ( if (warmup_complete[packet->cpu]) {
	     cout << "queue-packet-cpu " << queue->entry[index].cpu << endl;
